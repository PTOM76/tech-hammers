...
public class ElectricExcavatorItem extends GemExcavatorItem implements RcEnergyItem {
    ...
    public ElectricExcavatorItem(ToolMaterial toolMaterial, CompatibleItemSettings settings, int energyCapacity, RcEnergyTier tier, int cost, float poweredSpeed, float unpoweredSpeed) {
    ...
    public boolean isSuitableFor(BlockState state) {
        if (state == null) {
            return false;
        } else if (Items.DIAMOND_SHOVEL.isSuitableFor(state)) {
            return true;
        } else {
            return Items.DIAMOND_SHOVEL.getMiningSpeedMultiplier(new ItemStack(Items.DIAMOND_PICKAXE), state) > 1.0F;
        }
    }
